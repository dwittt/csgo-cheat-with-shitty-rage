...
void misc::thirdperson() {
    ...
    if (active_weapon->is_grenade()) {
        reset_thirdperson();
        return;
    }

    float distance = variables::thirdpersondistance + 5.f;       // Distance to thirdperson. Will be ajusted depending on walls behind us
    ...
    vec3_t eyes = csgo::local_player->get_eye_pos1();
