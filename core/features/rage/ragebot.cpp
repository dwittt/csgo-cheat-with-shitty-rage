...
bool ragebot::can_fire(player_t* target) {
...
bool ragebot::aimbot_weapon_check(bool check_scope) {
	...
	for (const std::pair<float, player_t*> item : target_list) {
		...
		for (const int hitbox : all_hitboxes) {
			...
			if (variables::aim::autowall.idx < 2) {
				...
				autowall_data = ragebot::autowall::handle_walls(csgo::local_player, entity, hitbox_pos, weapon_data, enabled_hitbox);
...
void ragebot::run_aimbot(c_usercmd* cmd) {
	if (!(variables::aim::autofire)	// Not holding aimbot key
...
void ragebot::auto_revolver(c_usercmd* cmd) {
