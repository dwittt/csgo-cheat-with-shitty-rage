...
float ragebot::autowall::get_damage_multiplier(int hit_group, float hs_multiplier) {
	switch (hit_group) {
...
bool ragebot::autowall::is_armored(int hit_group, bool helmet) {
	switch (hit_group) {
...
bool ragebot::autowall::trace_to_exit(trace_t& enter_trace, vec3_t& start, const vec3_t& direction, vec3_t& end, trace_t& exit_trace) {
...
static bool ragebot::autowall::handle_bullet_penetration(surface_data* enter_surface_data, trace_t& enter_trace, const vec3_t& direction, vec3_t& start, float penetration, float& damage) {
	trace_t exit_trace;
	vec3_t dummy;
...
// Used to check if target it visible or hittable. Used in ragebotbot.
// enabled_hitbox will be used to know what hitboxes are enabled by the user (cuz now its iterating all due to bodyragebot_if_lethal)
autowall_data_t ragebot::autowall::handle_walls(player_t* local_player, entity_t* entity, const vec3_t& destination, const weapon_info_t* weapon_data, bool enabled_hitbox) {
	...
	while (damage >= 1.0f && hits_left) {
		...
		//if (variables::ragebot::autowall.idx == 0) return { false, -1.f };
